...
def test_one_user():
    assert ltv._deprecated_compute_lifetime_value("../test/model/LTV=USD34.96.csv") == pytest.approx(34.965, 0.004)
...
def test_conversion_rate():
    ltv._deprecated_compute_lifetime_value("../test/model/LTV=USD34.96.csv")
    assert ltv.Conversion_percents == [1.0]

    ltv._deprecated_compute_lifetime_value("../test/model/three_users.csv")
...
def test_three_users():
    ...
    ltv._deprecated_compute_lifetime_value()
...
def test_functions_talk():
    ...
    #  [5,   5,    3,  3, 1, 1]
    assert ltv.compute_lifetime_value_using_numbers_of_user_retention(at_least_subscription, user_retention) == pytest.approx((5+3+3+1+1)*9.99*0.7 / num_of_trial_users, 0.001)
...
def test_integration_whole_system_total_ltv():
    ...
    assert 5 < ltv._deprecated_compute_lifetime_value() < 40
    print(ltv._deprecated_compute_lifetime_value())
