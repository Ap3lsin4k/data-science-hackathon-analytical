...
def test_compute_relative_user_conversion_rate():
    ...
    assert ltv.compute_conversion_rates_of_users_relative_to_previous_week([8, 4]) == [0.5]
    assert ltv.compute_conversion_rates_of_users_relative_to_previous_week([8, 4, 3]) == [0.5, 0.75]
    assert ltv.compute_conversion_rates_of_users_relative_to_previous_week([15, 13, 12, 11, 10, 3]) == [13 / 15, 12 / 13, 11 / 12, 10 / 11, 3 / 10]

    with pytest.raises(ValueError):
        ltv.compute_conversion_rates_of_users_relative_to_previous_week([3, 4])
